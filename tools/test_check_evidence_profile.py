...
ATTACKS = [
    ...
    ("doc band drifts from the Swift band",
     [(SVC, '"High": 12...16', '"High": 10...14')],
     "VOL-001 documents High"),
    ("band upper bound demands more exercise slots than VAR-001 allows",
     [(SVC, '"High": 12...16', '"High": 12...20'),
      (DOC, "High priority areas target `12-16`", "High priority areas target `12-20`")],
    ...
    ("allocator and validator ceilings disagree",
     [(SEL, "let maintenanceCeiling = recoveryTight ? 5.0 : 6.0",
           "let maintenanceCeiling = recoveryTight ? 8.0 : 10.0")],
     "do not agree"),
    ("owner-facing prose still quotes the old band",
     [(PAR, "Maintenance means roughly 4-6 quality sets per week",
           "Maintenance means roughly 6-10 quality sets per week")],
     "tell the owner"),
]

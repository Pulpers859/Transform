...
| `MAINT-001` | Non-priority maintenance band | Muscle groups outside the blueprint's priority list are maintenance work capped at roughly `10` direct sets/week (`8` when recovery or nutrition adherence is constrained) and floored at `4` (`3` when constrained); zero weekly direct sets for a major muscle group violates `BASE-001` and must be surfaced. The enforced floor sits below the `6-10` target band on purpose — `6` is the goal, `4` is where the word maintenance stops meaning anything, and a rule firing on every group merely sitting low would be ignored. A group below the floor may be short of exercise SLOTS or of SETS, and the two need different repairs: if its movements already sit at their role defaults the group needs another weekly exposure, but if they sit UNDER their defaults one of the allocator's ceilings refused sets it was entitled to spend — the role default, the group's maintenance ceiling, a priority's weekly target, a per-session direct cap, or the day's fatigue cap — and adding an exercise makes that day worse. (Session-time budgets are not among them: nothing refuses training for projected minutes.) This entry used to assert the first case unconditionally; the 2026-09-08 week was the second (Calves at 2 sets from one `.accessory` whose week-1 role default is 3) | Low-Moderate | Priority-only volume policing let non-priority volume grow unbounded (~98 weekly sets in a deficit) while hamstrings received zero sets undetected; a maintenance band of roughly 6-10 sets preserves tissue without stealing recovery from priorities, though the exact ceiling is a pragmatic coaching number rather than a settled literature value |

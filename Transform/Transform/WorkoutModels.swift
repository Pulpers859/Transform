...
struct WorkoutProgramScaffoldResponse: Codable {
    let programName: String
    let programSummary: String
    let splitType: String
    let daysPerWeek: Int
    let days: [WorkoutDayScaffoldResponse]
    ...
    init(
        ...
        splitType: String,
        daysPerWeek: Int,
        days: [WorkoutDayScaffoldResponse]
    ) {
        self.programName = programName
        self.programSummary = programSummary
        self.splitType = splitType
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let decodedDays = container.decodeFlexibleArray(WorkoutDayScaffoldResponse.self, forKey: .days)
        let trainingDayCount = decodedDays.filter { !$0.isRestDay }.count
        let fallbackDaysPerWeek = trainingDayCount > 0 ? trainingDayCount : 5
...
struct WorkoutDayScaffoldResponse: Codable {
    let dayNumber: Int
    let dayName: String
    let muscleGroups: String
    let isRestDay: Bool
    let sessionFocus: String
    let exercises: [WorkoutExerciseScaffoldResponse]
    ...
    init(
        ...
        isRestDay: Bool,
        sessionFocus: String,
        exercises: [WorkoutExerciseScaffoldResponse]
    ) {
        self.dayNumber = dayNumber
        self.dayName = dayName
        self.muscleGroups = muscleGroups
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let parsedExercises = container.decodeFlexibleArray(WorkoutExerciseScaffoldResponse.self, forKey: .exercises)
        let parsedDayNumber = container.decodeFlexibleInt(forKey: .dayNumber, default: 1, minimum: 1)
        let explicitRest = container.decodeFlexibleBool(forKey: .isRestDay)
        let resolvedRestDay = explicitRest ?? parsedExercises.isEmpty
        ...
        self.isRestDay = resolvedRestDay
        self.sessionFocus = container.decodeFlexibleString(
            forKey: .sessionFocus,
            default: resolvedRestDay
...
struct WorkoutExerciseScaffoldResponse: Codable {
    let exerciseName: String
    let sets: Int
    let reps: String
    let tempo: String
...
struct WorkoutDayDetailResponse: Codable {
    let dayNumber: Int
    let notes: String
    let exercises: [WorkoutExerciseDetailResponse]
    ...
    init(dayNumber: Int, notes: String, exercises: [WorkoutExerciseDetailResponse]) {
        self.dayNumber = dayNumber
        self.notes = notes
        self.exercises = exercises
    }
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.dayNumber = container.decodeFlexibleInt(forKey: .dayNumber, default: 1, minimum: 1)
        self.notes = container.decodeFlexibleString(
            forKey: .notes,
            default: "Execute with controlled setup, use the prescribed warm-up, and progress only if rep quality stays high."
        )
        self.exercises = container.decodeFlexibleArray(WorkoutExerciseDetailResponse.self, forKey: .exercises)
    }
}

struct WorkoutExerciseDetailResponse: Codable {
    let exerciseName: String
    let notes: String
    ...
    init(exerciseName: String, notes: String) {
        self.exerciseName = exerciseName
        self.notes = notes
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        self.notes = container.decodeFlexibleString(
            forKey: .notes,
            default: "Control the eccentric, own the target reps, and progress only if technique stays crisp."
        )
    }
}

...
extension ClaudeService {
    ...
    enum ReportedJointStressArea {
        case elbow
        case lowerBack
        case knee
    }

    enum JointStressMovementFamily: CaseIterable, Hashable {
        case curl
    ...
    func hasReportedRisk(
        for joint: ReportedJointStressArea,
        injuryRiskFocus: String
    ) -> Bool {
        let reported = normalizedPriorityText(injuryRiskFocus)
        let jointPhrases: [String]
        switch joint {
        case .elbow:
            jointPhrases = ["elbow"]
        case .lowerBack:
            jointPhrases = ["lower back", "low back", "lumbar", "lumbosacral"]
        case .knee:
            jointPhrases = [
                "knee", "patella", "patellar", "meniscus", "meniscal",
                "acl", "mcl", "pcl", "lcl"
            ]
        }
        guard containsPriorityPhrase(in: reported, keywords: jointPhrases) else { return false }

        return containsAny(
            reported,
            keywords: [
                "pain", "painful", "hurt", "ache", "discomfort", "injur", "irritat",
                "flare", "tender", "sore", "tight", "strain", "sprain", "instability",
                "surgery", "repair", "reconstruct", "tear", "rupture", "swelling",
                "aggravat", "bothers", "history", "tendin", "herniat", "fracture", "degenerat"
            ]
        )
    }

    func jointStressPhrases(for family: JointStressMovementFamily) -> [String] {
        switch family {
        case .curl:
            return ["curl", "curling"]
        case .tricepsExtension:
            return [
                "triceps extension", "tricep extension", "extension",
                "pressdown", "pushdown", "skull crusher"
            ]
        case .closeGripPress:
            return ["close grip", "close-grip", "jm press"]
        case .dip:
            return ["dip"]
        case .verticalPress:
            return [
                "overhead press", "overhead pressing", "press overhead", "pressing overhead",
                "shoulder press", "military press", "strict press", "vertical press"
            ]
        case .inclinePress:
        ...
        case .hinge:
            return ["deadlift", "rdl", "romanian deadlift", "hinge", "good morning"]
        case .squat:
            return ["squat", "squatting"]
        case .row:
            return ["row", "rowing"]
        case .splitSquat:
            return ["split squat", "bulgarian"]
        case .lunge:
            return ["lunge", "lunging"]
        case .legPress:
            return ["leg press"]
        case .kneeExtension:
            return ["leg extension", "knee extension"]
        }
    }

    func jointStressMovementFamily(
        for joint: ReportedJointStressArea,
        metadata: ExerciseMetadata
    ) -> JointStressMovementFamily? {
        let name = normalizedPriorityText(metadata.canonicalName)
        ...
        switch joint {
        case .elbow:
            if pattern.contains("curl") && containsAny(primary, keywords: ["bicep", "brachialis", "forearm"]) {
                return .curl
            }
            if pattern.contains("close-grip press") || name.contains("jm press") {
                return .closeGripPress
            }
            if containsAny(pattern, keywords: ["extension", "pressdown"]) && primary.contains("tricep") {
        ...
        case .lowerBack:
            if pattern.contains("hinge") {
                return .hinge
            }
            if pattern.contains("squat") {
                return .squat
            }
        ...
        case .knee:
            ...
            if pattern.contains("extension") && containsAny(primary, keywords: ["quad", "glute"]) {
    ...
    func reportedJointPainImplicates(
        _ joint: ReportedJointStressArea,
        exerciseName: String,
        muscleTarget: String,
        injuryRiskFocus: String
    ) -> Bool {
    ...
    ///
    /// Every charge is evidence-bound: a movement contributes only when the user's saved pain
    /// history, current check-in, or analysis reports that joint and reaches this movement. A
    /// vague joint report stays broadly cautious; a specific report stays on its named family.
    ///
    ...
    func exerciseJointStress(
        ...
        let elbowIsReported = reportedJointPainImplicates(
            .elbow,
            exerciseName: exercise.exerciseName,
            muscleTarget: exercise.muscleTarget,
            injuryRiskFocus: injuryRiskFocus
        ...
        let lowerBackIsReported = reportedJointPainImplicates(
            .lowerBack,
            exerciseName: exercise.exerciseName,
            muscleTarget: exercise.muscleTarget,
            injuryRiskFocus: injuryRiskFocus
        ...
        let kneeIsReported = reportedJointPainImplicates(
            .knee,
            exerciseName: exercise.exerciseName,
            muscleTarget: exercise.muscleTarget,
            injuryRiskFocus: injuryRiskFocus
    ...
    func validateJointStressBudget(on day: WorkoutDayResponse, injuryRiskFocus: String) -> [String] {
        ...
        let shoulderBudget = 12.0
        if stress.shoulder > shoulderBudget {
            issues.append(
                "Day \(day.dayNumber) accumulates excessive shoulder joint stress (\(String(format: "%.0f", stress.shoulder)) vs \(String(format: "%.0f", shoulderBudget)) budget). Swap a compound press for a machine or cable alternative, or drop a redundant pressing slot."
            )
        }
        if stress.elbow > 14 {
            issues.append(
    ...
    static let exerciseMetadataEntriesCache: [ExerciseMetadata] = [
            ...
            ExerciseMetadata(canonicalName: "Dumbbell Arnold Press", primaryAreas: ["Anterior Deltoids"], secondaryAreas: ["Lateral Deltoids", "Triceps"], movementPattern: "Vertical Press", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Cable Face Pull", primaryAreas: ["Rear Deltoids"], secondaryAreas: ["Upper Back"], movementPattern: "Face Pull", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Cable Face Pull with External Rotation", primaryAreas: ["Rear Deltoids"], secondaryAreas: ["Upper Back"], movementPattern: "Face Pull", fatigueCost: 1),
            ...
            ExerciseMetadata(canonicalName: "EZ-Bar Preacher Curl", primaryAreas: ["Biceps"], secondaryAreas: ["Forearms"], movementPattern: "Curl", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Cable Curl", primaryAreas: ["Biceps"], secondaryAreas: ["Forearms"], movementPattern: "Curl", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Dumbbell Hammer Curl", primaryAreas: ["Brachialis"], secondaryAreas: ["Biceps", "Forearms"], movementPattern: "Curl", fatigueCost: 1),
            ...
            ExerciseMetadata(canonicalName: "Close-Grip Barbell Bench Press", primaryAreas: ["Triceps"], secondaryAreas: ["Chest", "Anterior Deltoids"], movementPattern: "Close-Grip Press", fatigueCost: 2),
            ExerciseMetadata(canonicalName: "EZ-Bar JM Press", primaryAreas: ["Triceps"], secondaryAreas: ["Chest", "Anterior Deltoids"], movementPattern: "Close-Grip Press", fatigueCost: 2),
            ExerciseMetadata(canonicalName: "EZ-Bar Skull Crusher", primaryAreas: ["Triceps"], secondaryAreas: [], movementPattern: "Extension", fatigueCost: 1),
            ...
            ExerciseMetadata(canonicalName: "Barbell Romanian Deadlift", primaryAreas: ["Hamstrings"], secondaryAreas: ["Glutes", "Posterior Chain"], movementPattern: "Hinge", fatigueCost: 3),
            ...
            ExerciseMetadata(canonicalName: "Dumbbell Walking Lunge", primaryAreas: ["Quads/Glutes"], secondaryAreas: ["Quads", "Hamstrings"], movementPattern: "Lunge", fatigueCost: 2),
            ExerciseMetadata(canonicalName: "Dumbbell Bulgarian Split Squat", primaryAreas: ["Quads/Glutes"], secondaryAreas: ["Quads"], movementPattern: "Split Squat", fatigueCost: 2),
            ExerciseMetadata(canonicalName: "Hack Squat", primaryAreas: ["Quads"], secondaryAreas: ["Glutes"], movementPattern: "Squat", fatigueCost: 2),
            ExerciseMetadata(canonicalName: "Machine Leg Curl", primaryAreas: ["Hamstrings"], secondaryAreas: [], movementPattern: "Curl", fatigueCost: 1),
            ...
            ExerciseMetadata(canonicalName: "Machine Leg Extension", primaryAreas: ["Quads"], secondaryAreas: [], movementPattern: "Extension", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Barbell Hip Thrust", primaryAreas: ["Glutes"], secondaryAreas: ["Hamstrings"], movementPattern: "Hip Thrust", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Cable Pallof Press", primaryAreas: ["Obliques"], secondaryAreas: ["Anterior Core"], movementPattern: "Anti-Rotation", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Dumbbell Farmer's Walk", primaryAreas: ["Anterior Core"], secondaryAreas: ["Forearms", "Upper Back"], movementPattern: "Carry", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Landmine Meadows Row", primaryAreas: ["Upper Back", "Lats"], secondaryAreas: ["Biceps"], movementPattern: "Row", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Dumbbell Spider Curl", primaryAreas: ["Biceps"], secondaryAreas: [], movementPattern: "Curl", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Bayesian Cable Curl", primaryAreas: ["Biceps"], secondaryAreas: [], movementPattern: "Curl", fatigueCost: 1),
            ExerciseMetadata(canonicalName: "Dumbbell Concentration Curl", primaryAreas: ["Biceps"], secondaryAreas: [], movementPattern: "Curl", fatigueCost: 1),
            ...
            ExerciseMetadata(canonicalName: "Barbell Stiff-Leg Deadlift", primaryAreas: ["Hamstrings"], secondaryAreas: ["Glutes", "Posterior Chain"], movementPattern: "Hinge", fatigueCost: 3),
            ExerciseMetadata(canonicalName: "Dumbbell Single-Leg Romanian Deadlift", primaryAreas: ["Hamstrings"], secondaryAreas: ["Glutes"], movementPattern: "Hinge", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Barbell Good Morning", primaryAreas: ["Hamstrings"], secondaryAreas: ["Glutes", "Posterior Chain"], movementPattern: "Hinge", fatigueCost: 3),
            ...
            ExerciseMetadata(canonicalName: "Barbell Sumo Deadlift", primaryAreas: ["Glutes"], secondaryAreas: ["Quads", "Hamstrings", "Posterior Chain"], movementPattern: "Hinge", fatigueCost: 3),
            ExerciseMetadata(canonicalName: "Dumbbell Reverse Lunge", primaryAreas: ["Glutes"], secondaryAreas: ["Quads"], movementPattern: "Lunge", fatigueCost: 2),
            ...
            ExerciseMetadata(canonicalName: "Dumbbell Suitcase Carry", primaryAreas: ["Obliques"], secondaryAreas: ["Forearms"], movementPattern: "Carry", fatigueCost: 2)
    ]

    static let exerciseNameAliasCache: [String: String] = {
        let entries: [(String, String)] = [
            // Disambiguation forward-aliases: the pre-rename names, so a model still
            // emitting "Hammer Curl" resolves to the explicit catalog entry instead of
            // creating a second identity. Kept in step with
            // ExerciseNameDisambiguation.renames, which is the source of truth.
            ...
            ("Face Pull with External Rotations", "Cable Face Pull with External Rotation"),
            ("Farmer's Walk", "Dumbbell Farmer's Walk"),
            ("Farmer's Walks", "Dumbbell Farmer's Walk"),
            ...
            ("Single-Leg Romanian Deadlifts", "Dumbbell Single-Leg Romanian Deadlift"),
            ...
            ("Suitcase Carrys", "Dumbbell Suitcase Carry"),
            ("Sumo Deadlift", "Barbell Sumo Deadlift"),
            ("Sumo Deadlifts", "Barbell Sumo Deadlift"),
            ...
            ("Dead Bugs", "Dead Bug"),
            ("Suitcase Carries", "Dumbbell Suitcase Carry"),

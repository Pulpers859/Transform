...
**Current troubleshooting checkpoint:** `5a77c06` on `main`, synchronized with `origin/main`. Generator Tests run `29554714005` and Swift/Xcode run `29554714003` both passed. Live workflow run `29555834641` also passed: the workout and one-week nutrition checks each succeeded on their first Anthropic attempt. New nutrition generation makes one 7-day request and does not automatically generate Weeks 2–4. The live workflow has since been split into independent workout and nutrition jobs so a nutrition-only validation cannot spend workout credits.
